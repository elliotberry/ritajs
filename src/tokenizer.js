...
class Tokenizer {
  ...
  untokenize(arr, delim = " ") {
    ...
    for (let i = 1; i < arr.length; i++) {
      ...
      const thisNBPunct =
        NOSP_BF_PUNCT_RE.test(thisToken) ||
        UNTAG_RE[2].test(thisToken) ||
        LINEBREAK_RE.test(thisToken) // NB -> no space before the punctuation (add closing tag)
      ...
      const lastNBPunct =
        NOSP_BF_PUNCT_RE.test(lastToken) || LINEBREAK_RE.test(lastToken) // NB -> no space before
      const lastNAPunct =
        NOSP_AF_PUNCT_RE.test(lastToken) ||
        UNTAG_RE[1].test(lastToken) ||
        LINEBREAK_RE.test(lastToken) // NA -> no space after (add opening tag)
      const lastLB = LB_RE.test(lastToken)
      const lastRB = RB_RE.test(lastToken)
      const lastEndWithS =
        lastToken[lastToken.length - 1] === "s" &&
        lastToken != "is" &&
        lastToken != "Is" &&
        lastToken != "IS"
      ...
      const nextIsS =
        i == arr.length - 1 ? false : arr[i + 1] === "s" || arr[i + 1] === "S"
      ...
      if ((lastToken === "." && isDomain) || nextNoSpace) {
        ...
        if (withinQuote) {
          ...
          !(
            (APOS_RE.test(thisToken) && lastEndWithS) ||
  ...
  popTags(result, tags) {
    for (let i = 0; i < result.length; i++) {
      ...
      if (
        result[i].includes("_") &&
        !EMAIL_RE.test(result[i]) &&
        !URL_RE.test(result[i])
      ) {
  ...
  tagSubarrayToString(array) {
    ...
    let inspectIdx = 1
    while (
      inspectIdx < array.length - 1 &&
      TAGSTART_RE.test(array[inspectIdx])
    ) {
    ...
    return (
      start +
      this.untokenize(array.slice(contentStartIdx, contentEndIdx + 1)).trim() +
      end
    )
  }
}
...
const TAG = "TAG"
const UNDER_RE =
  /([0-9a-zA-z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u00FF]|[\.\,])_([0-9a-zA-z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u00FF])/g
const LB_RE = /^[\[\(\{\u27e8]+$/
const RB_RE = /^[\)\]\}\u27e9]+$/
const QUOTE_RE = /^[""\u201c\u201d\u2019\u2018`''\u00ab\u00bb]+$/
...
const TOKENIZE_RE = [
  ...
  "_$1$2_", // Pf.
  /\b([Ii])([Nn])([Dd]|[Cc])[\.]/g,
  "_$1$2$3_", // Ind. and Inc.
  /\b([Cc])([Oo])[\.][\,][\s]([Ll])([Tt])([Dd])[\.]/g,
  "_$1$2dcs$3$4$5_", // co., ltd.
  /\b([Cc])([Oo])[\.][\s]([Ll])([Tt])([Dd])[\.]/g,
  "_$1$2ds$3$4$5_", // co. ltd.
  /\b([Cc])([Oo])[\.][\,]([Ll])([Tt])([Dd])[\.]/g,
  "_$1$2dc$3$4$5_", // co.,ltd.
  /\b([Cc])([Oo])([Rr]?)([Pp]?)[\.]/g,
  "_$1$2$3$4_", // Corp. and Co.
  /\b([Ll])([Tt])([Dd])[\.]/g,
  "_$1$2$3_", // ltd.
  /\b(prof|Prof|PROF)\./g,
  "_$1_", //Prof.
  ...
  "$2COLON$3$4$5", //urls with http(s)
  //decimal #
  /([\-]?[0-9]+)\.([0-9]+)/g,
  "$1DECIMALDOT$2_", //(-)27.3
  /([\-]?[0-9]+)\.([0-9]+)e([\-]?[0-9]+)/g,
  "_$1DECIMALDOT$2POWERE$3_", //(-)1.2e10
  /([0-9]{1,3}),([0-9]{3})/g,
  "$1_DECIMALCOMMA_$2", // large numbers like 19,700 or 200,000,000.13
  /([A-Za-z0-9])\.([A-Za-z0-9])/g,
  "$1_DECIMALDOT_$2", //www.example.com

  //escape sequences of line breaks in ASCII
  /\r\n/g,
  " _CARRIAGERETURNLINEFEED_ ", // CR LF
  /\n\r/g,
  " _LINEFEEDCARRIAGERETURN_ ", // LF CR
  /\n/g,
  " _LINEFEED_ ", // LF
  /\r/g,
  " _CARRIAGERETURN_ ", // CR
  ...
  /_([Cc])([Oo])([Rr]?)([Pp]?)_/g,
  "$1$2$3$4.", // Corp. and Co.
  /_([Cc])([Oo])dc([Ll])([Tt])([Dd])_/g,
  "$1$2.,$3$4$5.", // co.,ltd.
  /_([Ll])([Tt])([Dd])_/g,
  "$1$2$3.", // ltd.
  /_([Cc])([Oo])dcs([Ll])([Tt])([Dd])_/g,
  "$1$2.,_$3$4$5.", // co., ltd.
  /_([Cc])([Oo])ds([Ll])([Tt])([Dd])_/g,
  "$1$2._$3$4$5.", // co. ltd.
  /_(prof|PROF|Prof)_/g,
  "$1.", //Prof.
  /([\-]?[0-9]+)DECIMALDOT([0-9]+)_/g,
  "$1.$2", //(-)27.3
  /_([\-]?[0-9]+)\DECIMALDOT([0-9]+)POWERE([\-]?[0-9]+)_/g,
  "$1.$2e$3", //(-)1.2e(-)9
  /_DECIMALCOMMA_/g,
  ",", // large numbers like 200,000,000.13
  /_DECIMALDOT_/g,
  ".",
  // /(\w+([\.-]?\w+)*)__AT__(\w+([\.-]?\w+)*)\.(\w{2,3})/g, "$1@$3.$5",
  /__AT__/g,
  "@",
  /((http[s]?)COLON(\/\/))([-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b)([-a-zA-Z0-9()@:%_\+.~#?&\/\/=]*)/g,
  "$2:$3$4$5",
  /_LINEFEED_/g,
  "\n", // LF
  /_CARRIAGERETURN_/g,
...
const CONTRACTS_RE = [
  ...
  "$1 had", // changed from would, 12/8/23
...
const TAG_RE =
  /(<\/?[a-z][a-z0-9='"#;:&\s\-\+\/\.\?]*\/?>|<!DOCTYPE[^>]*>|<!--[^>-]*-->)/i // html tags (rita#103)
