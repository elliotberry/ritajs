...
class Lexicon {
  ...
  _matchPos(word, rdata, opts, strict) {
    ...
    if (result !== word) {
      if (opts.numSyllables) {
        ...
        const syls = this.analyzer.analyzeWord(result, { silent: true }).syllables;
  ...
  async _bySoundAndLetter(word, opts) {
    ...
    const promises = types.map(type => this._promise
  ...
  _parseRegex(regex, opts) {
    ...
    if (typeof regex === 'string') {
      if (opts && opts.type === 'stresses' && /^\^?[01]+\$?$/.test(regex)) { // keep ^ and $ in regex like "^101$"
      ...
      if (typeof regex === 'string') {
        if (opts && /^stress(es)?$/.test(opts.type) && /^\^?[01]+\$?$/.test(regex)) {
          regex = regex.replace(/([01])(?=([01]))/g, "$1/");
        }
        regex = new RegExp(regex);
      }
