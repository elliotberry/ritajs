...
class Lexicon {
  ...
  hasWord(word, opts = {}) {
    ...
    const exists = this.data.hasOwnProperty(token)
    const noDerivations = opts.noDerivations
    ...
    const sing = this.RiTa.singularize(token)
    if (this.data.hasOwnProperty(sing)) {
      const tags = this.RiTa.tagger.allTags(sing)
    ...
    const vlemma = this.RiTa.conjugator.unconjugate(token, opts)
    if (vlemma && this.data.hasOwnProperty(vlemma)) {
      const tags = this.RiTa.tagger.allTags(vlemma)
  ...
  rhymesSync(theWord, opts = {}) {
    ...
    for (; i < words.length; i++) {
      ...
      const phones = data ? data[0] : this.rawPhones(word)
  ...
  randomWord(pattern, opts) {
    ...
    if (!(pattern instanceof RegExp) && typeof pattern === "object" && !opts) {
  ...
  isAlliteration(word1, word2) {
    ...
    const c1 = this._firstPhone(this._firstStressedSyl(word1))
    const c2 = this._firstPhone(this._firstStressedSyl(word2))
  ...
  isRhyme(word1, word2) {
    ...
    const p1 = this._lastStressedVowelPhonemeToEnd(word1)
    const p2 = this._lastStressedVowelPhonemeToEnd(word2)
    return p1 && p2 && p1 === p2
  }
  ...
  _byTypeSync(theWord, opts) {
    ...
    const phonesA =
      matchSound ? this._toPhoneArray(this.rawPhones(input)) : input
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      const med = this.minEditDist(phonesA, phonesB)
  ...
  _matchPos(word, rdata, opts, strict) {
    const posArr = rdata[1].split(" ")

    // check the pos here (based on strict flag)
    if (
      !posArr.includes(opts.targetPos) ||
    ...
    if (result !== word) {
      if (opts.numSyllables) {
        ...
        const syls = this.analyzer.analyzeWord(result, {
          silent: true
        }).syllables
        const num = syls.split(this.RiTa.SYLLABLE_BOUNDARY).length
  ...
  _checkCriteria(word, rdata, opts) {
    ...
    if (opts.numSyllables) {
      const syls = rdata[0].split(" ").length
  ...
  _bySoundAndLetterSync(word, opts) {
    const bySound = this._byTypeSync(word, { ...opts, type: "sound" })
    const byLetter = this._byTypeSync(word, { ...opts, type: "letter" })
  ...
  async _bySoundAndLetter(word, opts) {
    const types = ["sound", "letter"] // in parallel
    const promises = types.map((type) =>
      this._promise(this._byTypeSync, [word, { ...opts, type }])
    )
  ...
  rawPhones(word, opts) {
    const noLts = opts && opts.noLts
    const fatal = opts && opts.fatal
    const rdata = this._lookupRaw(word, fatal)
    if (rdata && rdata.length) return rdata[0]

    if (!noLts) {
      const phones = this.RiTa.analyzer.computePhones(word)
  ...
  _parseRegex(regex, opts) {
    ...
    if (typeof regex === "string") {
      if (opts && opts.type === "stresses" && /^\^?[01]+\$?$/.test(regex)) {
        // keep ^ and $ in regex like "^101$"
      ...
      typeof regex === "object" ||
      (regex === undefined && typeof opts === "object")
    ) {
      if (!opts) {
  ...
  _regexMatch(word, data, regex, type) {
    if (type === "stresses") {
      const phones = data ? data[0] : this.rawPhones(word)
      const stresses = this.analyzer.phonesToStress(phones)
  ...
  _firstPhone(rawPhones) {
    if (rawPhones && rawPhones.length) {
      const phones = rawPhones.split(this.RiTa.PHONE_BOUNDARY)
  ...
  _lastStressedVowelPhonemeToEnd(word) {
    if (word && word.length) {
      const raw = this._lastStressedPhoneToEnd(word)
      if (raw) {
        let idx = -1
