...
class Conjugator {
  constructor(parent) {
    ...
    this.allVerbs = Object.keys(data).filter((word) =>
      data[word][1].split(" ").includes("vb")
    )
    ...
    // 3) ends with -ing
      ...
      if (/([a-z])\1ing$/.test(word)) {
        if (this.verbsEndingInDouble.includes(word.slice(0, -3))) {
          dbug &&
            console.log(`'${word}' hit rule: ends with -(XX)ing [in-list]`)
          return word.slice(0, -3)
        }
      ...
      if (
        word.endsWith("ying") &&
        this.verbsEndingInE.includes(word.replace(/ying$/, "ie"))
      ) {
...
const PAST_PART_RULES = [
  ...
  RE(
    `^${VERBAL_PREFIX}?(take|rise|strew|blow|draw|drive|know|give|arise|gnaw|grave|grow|hew|know|mow|see|sew|throw|prove|saw|quartersaw|partake|sake|shake|shew|show|shrive|sightsee|strew|strive)$`,
    0,
    "n"
  ),
  ...
  // Null past forms
  RE(
    `^${VERBAL_PREFIX}?(cast|thrust|typeset|cut|bid|upset|wet|bet|cut|hit|hurt|inset|let|cost|burst|beat|beset|set|upset|hit|offset|put|quit|wed|typeset|wed|spread|split|slit|read|run|rerun|shut|shed)$`,
    0
  )
]
...
const PAST_RULES = [
  ...
  // Null past forms
  RE(
    `^${VERBAL_PREFIX}?(cast|thrust|typeset|cut|bid|upset|wet|bet|cut|hit|hurt|inset|let|cost|burst|beat|beset|set|upset|offset|put|quit|wed|typeset|wed|spread|split|slit|read|run|shut|shed|lay)$`,
    0
  )
]
